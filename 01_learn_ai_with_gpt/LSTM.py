...
sequence_length = 800  # Set sequence length to 30
...
# LSTM 모델 생성
model = Sequential()
model.add(Bidirectional(LSTM(64, input_shape=(X.shape[1], 1), return_sequences=True)))
...
# LSTM 모델 학습
model.fit(X.reshape(X.shape[0], X.shape[1], 1), y, epochs=100, batch_size=32, validation_split=0.2)

# 장기적인 예측을 위해 마지막 시퀀스 데이터로부터 미래를 예측
forecast_steps = 300
last_sequence = df['close_scaled'].values[-sequence_length:]
forecast = []
# (970, 29, 1) 12313123123
